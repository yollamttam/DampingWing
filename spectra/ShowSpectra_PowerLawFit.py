...
def plotSpectra(sFilename,nFilename,z):
    ...
    while (acceptable==0):
        ...
        smin = tmin*(1+z)
        smax = tmax*(1+z)
        lambdaR = np.mean(np.array([smin,smax]))
        FR = np.mean(flux[(lambdas>smin)&(lambdas<smax)])
        PLfit = FR*(lambdas/lambdaR)**(-1.56)
    ...
    PLfitA = PLfit[(lambdas>LAmin)&(lambdas<LAmax)]
    PLfitB = PLfit[(lambdas>LBmin)&(lambdas<LBmax)]
    ...
    outputFileA ="%s_LyaMatrix_PLfit%s" % (fileBase, fileExt)
    outputFileB ="%s_LybMatrix_PLfit%s" % (fileBase, fileExt)
